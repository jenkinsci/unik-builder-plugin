...
public class AttachVolumeCommand extends UnikCommand {
    ...
    public static class AttachVolumeCommandDescriptor extends UnikCommandDescriptor {
        ...
        @POST
        public FormValidation doCheckInstanceId(@QueryParameter String instanceId, @AncestorInPath Item item) {
            if (item == null) {
                return FormValidation.ok();
            }
            item.checkPermission(Item.CONFIGURE);
        ...
        @POST
        public FormValidation doCheckMountPoint(@QueryParameter String mountPoint, @AncestorInPath Item item) {
            if (item == null) {
                return FormValidation.ok();
            }
            item.checkPermission(Item.CONFIGURE);
